...
def collect_data(firstCurrency=None,dateTimeInput1=None):
    ...
    results = session.query(Crypto_Table.symbol, Crypto_Table.price,cast(Crypto_Table.crypto_timestamp, Date),extract('hour', cast(Crypto_Table.crypto_timestamp, DateTime)), extract('minute', cast(Crypto_Table.crypto_timestamp, DateTime)))\
    .filter(Crypto_Table.symbol == firstCurrency).\
    filter(cast(Crypto_Table.crypto_timestamp, DateTime) == dateTimeInput1).\
      group_by(Crypto_Table.symbol,Crypto_Table.price,cast(Crypto_Table.crypto_timestamp, Date),extract('hour', cast(Crypto_Table.crypto_timestamp, DateTime)), extract('minute', cast(Crypto_Table.crypto_timestamp, DateTime)))\
    .limit(1000).all()
    # filter(cast(Crypto_Table.crypto_timestamp, Timestamp) <= dateTimeInput2).distinct().all()
    test = list(np.ravel(results))
    return json.dumps(test, default=alchemyencoder)
