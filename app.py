...
def stations():
    conn = engine.connect()
    results = conn.execute("SELECT  symbol,price,max(date_format(crypto_timestamp,"%Y-%m-%dT%H:%i:00")) as crypto_timestamp FROM crypto  group by symbol,price order by symbol desc")
