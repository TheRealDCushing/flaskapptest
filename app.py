...
def stations():
    conn = engine.connect()
    results = conn.execute("SELECT  symbol,max(price) as price,max(Crypto_Table.crypto_timestamp) as crypto_datetime FROM crypto  group by symbol order by symbol desc")
